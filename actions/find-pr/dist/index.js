...
var require_lib3 = __commonJS({
  "../../node_modules/.pnpm/node-fetch@2.6.9/node_modules/node-fetch/lib/index.js"(exports, module) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
...
async function run() {
  const octokit = (0, import_github.getOctokit)((0, import_core.getInput)("github_token"));
  ...
  if (import_github.context.eventName === "push") {
    ...
    (0, import_core.setOutput)("source_repo", source_repo);
    (0, import_core.setOutput)("source_branch", source_branch);
    (0, import_core.setOutput)("pr_number", pr_number);
    (0, import_core.setOutput)("found_pr", !!(source_repo && source_branch && pr_number));
    return;
  } else if (import_github.context.eventName === "pull_request") {
    const source_repo = import_github.context.payload.pull_request?.head.repo.full_name;
    const source_branch = import_github.context.payload.pull_request?.head.ref;
    ...
    const [source_repo, source_branch, pr_number] = get_pr_details_from_number(
      open_pull_requests,
      import_github.context.payload.issue?.number
    );
    (0, import_core.setOutput)("source_repo", source_repo);
    (0, import_core.setOutput)("source_branch", source_branch);
    (0, import_core.setOutput)("pr_number", pr_number);
  ...
  if (!import_github.context.payload.workflow_run)
    return;
  if (import_github.context.payload.workflow_run.event === "pull_request" || import_github.context.payload.workflow_run.event === "push") {
    const [source_repo, source_branch, pr_number] = get_pr_details_from_refs(open_pull_requests);
    (0, import_core.setOutput)("source_repo", source_repo);
    (0, import_core.setOutput)("source_branch", source_branch);
    (0, import_core.setOutput)("pr_number", pr_number);
    (0, import_core.setOutput)("found_pr", !!(source_repo && source_branch && pr_number));
  } else if (import_github.context.payload.workflow_run.event === "issue_comment") {
    const title = import_github.context.payload.workflow_run?.display_title;
...
function get_pr_details_from_sha(pull_requests) {
  const head_sha = import_github.context.payload.head_commit?.id;
  const [source_repo, source_branch, pr_number] = pull_requests.map((pr) => [
    ...
    import_github.context.payload.repository?.full_name,
    import_github.context.payload.ref?.split("/").slice(2).join("/"),
    null
  ];
  return [source_repo, source_branch, pr_number];
}
...
function get_pr_details_from_refs(pull_requests) {
  const source_repo = import_github.context.payload.workflow_run?.head_repository?.full_name;
  const source_branch = import_github.context.payload.workflow_run?.head_branch;
